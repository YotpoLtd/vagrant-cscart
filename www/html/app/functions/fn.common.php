...
{
    if (fn_allowed_for('ULTIMATE:FREE')) {
        $currencies = fn_get_currencies_list(AREA, CART_LANGUAGE, array('only_primary' => 'Y'));
    } else {
        $currencies = fn_get_currencies_list(AREA, CART_LANGUAGE, array('status' => array('A')));
    }
 ...
 * @return array Currencies list
 */
function fn_get_currencies_list($area = AREA, $lang_code = CART_LANGUAGE, $params = array())
{
    $cond = $join = $order_by = '';
